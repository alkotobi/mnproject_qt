...
bool testPostgresWithDef(){
    const MnTableDef def = {
            .table_name = "nour",
            .fields = {
                    {.field_name = "id",.field_type = DbTypes::INTEGER},
                    {.field_name = "integer_field",.field_type = DbTypes::INTEGER},
                    {.field_name = "text_field",.field_type = DbTypes::TEXT},
                    {.field_name = "real_field",.field_type = DbTypes::REAL},
                    {.field_name = "varchar_field",.field_type = DbTypes::VARCHAR,.field_length = 255},
                    {.field_name = "bool_field",.field_type = DbTypes::BOOL},
                    {.field_name = "datetime_field",.field_type = DbTypes::DATETIME}
            },
            .default_data = "",
            .description = "",
            .insert_sql = "",
    ...
    ret = ret && conn.isConnected();
    test(ret, "connect to db");
    MnTable qry(&conn, def, {});
    ret = ret && qry.exec("DROP TABLE IF EXISTS nour");
    test(ret, "drop table");
    ret = ret && conn.execCreateTableSql(def);
    test(ret, "create table");
    ret = ret && qry.open();
    ret = ret && qry.isOpen();
    test(ret, "mntable open");
    ...
    std::cout << "-------------------------\n";
    qry.close();
    qry = MnTable(&conn,def,"text_field=?",{"ALI"});
    ret = qry.open();
    test(ret,"open MnTable with params");
    qry.printAll();
    return ret;
